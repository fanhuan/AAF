...
void getoptions()
{
  ...
  do {
   ...
   "\n  Y to accept these or type the letter for one to change\n");
    ...
    if (!done) {
      if (((!usertree) && (strchr("DJOUNPKG-LRSM01234", ch) != NULL))
          || (usertree && ((strchr("DOUNPKG-LRSM01234", ch) != NULL)))){
        switch (ch) {
...
void inputoptions()
{
  ...
  if (power == (long)power)
    fprintf(outfile, "%2ld\n", (long)power);
/* output the kmer info to outfile? */
  else
  ...
  for (i = 0; i < spp; i++) {
    if (i + 1 != m && n[i] > 0) {
      if (WITH->d[i] < epsilonf)
        ...
        /*********************/
       /* kluged from fitch.c */
      WITH->w[i] = n[i] / ((1 - exp(-k*(WITH->d[i])))/exp(-k*(WITH->d[i])));
        /*********************/
        /*********************/
        /*********************/
        /*********************/
